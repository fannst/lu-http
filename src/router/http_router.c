...
int32_t http_router_use (http_router_t *router, http_socket_t *socket, const http_request_t *request, http_response_t *response, const char *path) {    
    ...
    // Loops over all the parts of the url, and starts matching the routes we have.
    char *save_ptr;
    ...
    while (token != NULL) {
        ...
        while (route != NULL) {
            ...
            if (route->type == HTTP_ROUTE_TYPE__CALLBACK) {
                ...
                ((http_route_callback) (route->data)) (socket, request, response, remaining_path, route->u);
